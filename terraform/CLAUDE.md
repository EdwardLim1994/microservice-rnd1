...
that module's `main.tf` for the actual `kubernetes_namespace`/`helm_release` resources.

## Adding a new app

Give `servers/<app>/terraform/` (or `frontends/<app>/terraform/`, `apps/<app>/terraform/`) the same
shape as `servers/demo1/terraform/`:
...
Once this root config manages an app (as it now does for demo1 and frontend1 — each one's state
was migrated in with `terraform state mv -state=../servers/<app>/terraform/terraform.tfstate
-state-out=terraform.tfstate module.<app>.<resource> module.<app>.<resource>`, once per resource),
treat that app's own `servers/<app>/terraform` (or `frontends/<app>/terraform`) as a
template/reference for *new* apps, not something to `apply` standalone anymore — its local state
file for that app's resources is now empty by design (moved here), so a stray `apply` there would
try to recreate them from scratch instead of erroring cleanly.

## Frontends are not servers — one real difference

`frontends/frontend1`'s only "config" (`PUBLIC_GRAPHQL_URL`) is baked into its client JS bundle at
*image build time* by Rsbuild, not read from env vars at pod start — see
`frontends/frontend1/terraform/CLAUDE.md`. Its `module/main.tf` therefore only has an
`image.app.tag` `set` block, nothing env-shaped — don't expect a frontend's terraform module to
grow `set` blocks the way `servers/demo1`'s did for Kafka; there's nothing at deploy time for it
to configure.
...
for each browser-facing Service (`portal` on `3000`, `frontend1` on `3001`, plus
`apollo-router` on `4000` in the `infra` namespace — see `services/terraform/CLAUDE.md`), bound to
`127.0.0.1` and auto-forwarded to Windows. Each Module Federation remote/host's address baked into
its bundle at image build time (`FRONTEND1_HOST`/`FRONTEND1_PORT`, `PUBLIC_GRAPHQL_URL`) must then
match the port-forward's local port, not the NodePort — see this repo root's `CLAUDE.md` for the
full end-to-end runbook including exact build-arg values used.
...
`rsbuild preview`, which only `output.assetPrefix` affects. See `frontends/frontend1/rsbuild.config.ts`
for the fix (also applied to the `turbo gen web` remote template).
...
confirmed hands-on switching frontend1's Service to NodePort: `terraform plan` reported "No
