...
// RegisterUseCase drives AuthentikClient.enroll(email, password) — a thin wrapper over the
// existing Admin API createUser() (same mechanism SignUpUseCase used, still bypassing Authentik's
// own enrollment-flow stages; see servers/auth/CLAUDE.md's "Known v1 limitations"), using the
// email itself as Authentik's username so a duplicate-email registration surfaces as a
// username-uniqueness violation. The mock below is a bare plain object exposing only a single
// `enroll(email, password)` method, cast to AuthentikClient.
