...
import GetPayslipURLUseCase from "./GetPayslipURLUseCase";
...
export default class GetPayslipURLGrpcUseCase extends BaseUseCase<GetPayslipURLRequest, PayslipDownloadURLMessage> {
	private readonly getPayslipURLUseCase: GetPayslipURLUseCase;

	constructor({ getPayslipURLUseCase }: { getPayslipURLUseCase: GetPayslipURLUseCase }) {
		super();
		this.getPayslipURLUseCase = getPayslipURLUseCase;
	}

	async execute(request: GetPayslipURLRequest): Promise<PayslipDownloadURLMessage> {
		const result = (await this.getPayslipURLUseCase.execute({
			input: { employeeId: request.employeeId, month: request.month, year: request.year },
