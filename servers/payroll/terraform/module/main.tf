resource "kubernetes_namespace" "payroll" {
  metadata {
    name = var.namespace
  }
}

resource "helm_release" "payroll" {
  name      = "payroll"
  ...
  # (source = "../servers/payroll/terraform/module"). abspath() normalizes the result so both
  ...
  namespace = kubernetes_namespace.payroll.metadata[0].name
