...
function createMockRepo(existing: Record<string, unknown> | null) {
	...
	return { repo: repo as unknown as NotificationRepository, marked: () => marked };
}
...
test("marks an existing notification as read", async () => {
	...
	const useCase = new MarkNotificationReadUseCase({ notificationRepository: repo });

	const result = (await useCase.execute({ id: "notif-1" })) as { read: boolean };
