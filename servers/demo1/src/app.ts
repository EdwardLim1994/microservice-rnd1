...
export default async function main() {
  const grpcServer = new GrpcServer({ port: 4001 }).controller(new DemoGrpcController())
  const graphqlServer = new GraphqlServer({ port: 5001, federation: true }).withController(new DemoGraphqlController("localhost:4001"))
