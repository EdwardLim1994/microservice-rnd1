...
servers/<name>/
  ...
  src/
    ...
    configs/
      proto/buf.gen.yaml       # buf codegen template (ts-proto, + protoc-gen-es if needed) — per-server, not shared
...
**`buf.yaml` is not per-server** — unlike `buf.gen.yaml` (per-server codegen template, differs in
`out:` paths and plugin list), `buf.yaml`'s `lint`/`breaking` rule config was byte-identical between
`demo1`/`demo2`, so it's consolidated into a single buf v2 **workspace** file at
`packages/config/src/buf/buf.yaml`, declaring both servers' proto directories as `modules:`. `buf.yaml`
has no `extends` mechanism (unlike `tsconfig.json`) — this works instead because `buf lint`/
`buf breaking` accept an explicit `--config <path>` pointing anywhere, and `buf.yaml`'s `modules[].path`
entries resolve relative to the `<input>` you run against (repo root, via `.`), not relative to the
config file's own location — hence `packages/config/src/buf/buf.yaml`'s module paths are repo-root-relative
(`servers/demo1/src/schemas/proto/`), not `../../`-relative to `packages/config`. Run from the repo
root: `buf lint . --config packages/config/src/buf/buf.yaml` (needs `PATH` to include a server's
`node_modules/.bin`, e.g. `servers/demo1/node_modules/.bin`, since `buf` isn't hoisted to the repo
root's own `node_modules/.bin`).
