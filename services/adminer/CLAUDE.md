...
- On the `adminer` Docker network — the same network every server's Postgres container joins (e.g.
  `<serverName>-db`, see that server's own `CLAUDE.md`), specifically so Adminer can reach them by
  service name. If a new server adds its own database, its `docker-compose.yml` must put that DB
  service on the `adminer` network too, or it won't be reachable from here.
- Login fields when connecting: System `PostgreSQL`, Server `<db-service-name>` (e.g.
  `<serverName>-db`), then that database's user/password/db name from its own
