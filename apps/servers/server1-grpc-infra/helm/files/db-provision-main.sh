...
# creates server1-grpc-secret, is app-chart/Tilt-owned, not this chart's) — nothing to patch, and
...
if kubectl get secret server1-grpc-secret -n "$APP_NAMESPACE" >/dev/null 2>&1; then
  ...
  kubectl annotate secret server1-grpc-secret -n "$APP_NAMESPACE" "vault.lease-id=$NEW_LEASE" --overwrite
