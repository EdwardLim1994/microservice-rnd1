...
#
# Fully-qualified, not bare "server1-redis" — this connection is made by Vault itself, running
# in a different namespace (services/vault's "infra") than server1-redis ("servers"), and Vault
...
vault write database/config/server1-redis \
  plugin_name=redis-database-plugin \
  host=server1-redis.servers.svc.cluster.local \
  port=6379 \
  tls=false \
  username=default \
  password="$REDIS_ADMIN_PASSWORD" \
  allowed_roles="server1-redis-role"

vault write database/roles/server1-redis-role \
  db_name=server1-redis \
  creation_statements='["+@all","~*"]' \
  default_ttl="$DB_DEFAULT_TTL" \
  max_ttl="$DB_MAX_TTL"

vault read database/creds/server1-redis-role > /tmp/creds
