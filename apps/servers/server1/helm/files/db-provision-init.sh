...
# Fully-qualified, not bare "server1-db" — this connection is made by Vault itself, running in
# a different namespace (services/vault's "infra") than server1-db ("servers"), and Vault
...
# so Vault's own DNS search domain (infra) can't find a bare "server1-db" that only exists in a
# different namespace.
vault write database/config/server1-postgresql \
  plugin_name=postgresql-database-plugin \
  connection_url="postgresql://{{username}}:{{password}}@server1-db.servers.svc.cluster.local:5432/server1?sslmode=disable" \
  username="myuser" \
  password="$PGADMIN_PASSWORD" \
  allowed_roles="server1-role"
...
# server1-migrate in db.yaml), and only membership (inheriting its grants) lets the new login
# read/write them.
vault write database/roles/server1-role \
  db_name=server1-postgresql \
...
# One read only — database/creds/server1-role is a dynamic-secret endpoint that mints a
...
vault read database/creds/server1-role > /tmp/creds
