...
PREV_LEASE=$(kubectl get secret server1-secret -o jsonpath='{.metadata.annotations.vault\.lease-id}' 2>/dev/null || true)

DATABASE_URL="postgresql://${NEW_USER}:${NEW_PASS}@server1-db:5432/server1"
B64_URL=$(printf '%s' "$DATABASE_URL" | base64 | tr -d '\n')
kubectl patch secret server1-secret --type merge \
  -p "{\"data\":{\"DATABASE_URL\":\"$B64_URL\"}}"
kubectl annotate secret server1-secret "vault.lease-id=$NEW_LEASE" --overwrite
