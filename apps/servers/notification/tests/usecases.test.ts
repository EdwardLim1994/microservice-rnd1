...
function makeRepo() {
  return {
    ...
    findByRecipient: mock(async () => ({ notifications: [], total: 0, unreadCount: 0 })),
...
test("ConsumeNotificationEventUseCase — persists event", async () => {
  const repo = makeRepo();
  const uc = new ConsumeNotificationEventUseCase({ notificationRepository: repo as never });
  await uc.execute({ recipientId: "u1", type: "LEAVE_REQUEST_RECEIVED", message: "received" });
  expect(repo.create).toHaveBeenCalledWith({ recipientId: "u1", type: "LEAVE_REQUEST_RECEIVED", message: "received" });
});

test("MyNotificationsUseCase — delegates to repo with defaults", async () => {
  const repo = makeRepo();
  const uc = new MyNotificationsUseCase({ notificationRepository: repo as never });
...
test("UnreadNotificationCountUseCase — returns count", async () => {
  const repo = makeRepo();
  const uc = new UnreadNotificationCountUseCase({ notificationRepository: repo as never });
...
test("MarkNotificationReadUseCase — marks single notification read", async () => {
  const repo = makeRepo();
  const uc = new MarkNotificationReadUseCase({ notificationRepository: repo as never });
...
test("MarkAllNotificationsReadUseCase — returns updated count", async () => {
  const repo = makeRepo();
  const uc = new MarkAllNotificationsReadUseCase({ notificationRepository: repo as never });
  const count = await uc.execute({ userId: "u1" });
