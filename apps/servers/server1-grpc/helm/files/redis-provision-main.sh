...
PREV_LEASE=$(kubectl get secret server1-grpc-redis-secret -o jsonpath='{.metadata.annotations.vault\.lease-id}' 2>/dev/null || true)

REDIS_URL="redis://${NEW_USER}:${NEW_PASS}@server1-grpc-redis:6379"
B64_URL=$(printf '%s' "$REDIS_URL" | base64 | tr -d '\n')
kubectl patch secret server1-grpc-redis-secret --type merge \
  -p "{\"data\":{\"REDIS_URL\":\"$B64_URL\"}}"
kubectl annotate secret server1-grpc-redis-secret "vault.lease-id=$NEW_LEASE" --overwrite
