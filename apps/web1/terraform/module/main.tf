resource "kubernetes_namespace" "web1" {
  metadata {
    name = var.namespace
  }
}

resource "helm_release" "web1" {
  name      = "web1"
  ...
  # "../apps/web1/terraform/module"). abspath() normalizes the result so both
  ...
  namespace = kubernetes_namespace.web1.metadata[0].name
