resource "kubernetes_namespace" "hr-portal" {
  metadata {
    name = var.namespace
  }
}

resource "helm_release" "hr-portal" {
  name      = "hr-portal"
  ...
  # "../apps/hr-portal/terraform/module"). abspath() normalizes the result so both
  ...
  namespace = kubernetes_namespace.hr-portal.metadata[0].name
