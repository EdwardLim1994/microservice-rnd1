---
title: CI/CD
description: GitHub Actions workflow-to-branch/event map implementing this repo's branching and tagging strategy
---
...
| `ci-sonar-main.yml` | `push` to `main` | Re-runs the full (unfiltered) test suite and a SonarCloud scan against `main` itself. Exists because `ci-verify.yml` only ever scans in PR context — without this, `main`'s own SonarCloud branch analysis never updates after a merge, even though every PR into it passed its own scan. |
...
| `SONAR_TOKEN` | secret | `ci-verify.yml`, `ci-sonar-main.yml` — SonarCloud project already keyed via root `sonar-project.properties` (`EdwardLim1994_microservice-rnd1`) |
...
- **No package defines a `"lint"` script anywhere in the repo**, despite `turbo.json` declaring a
  ...
  `check` (Biome's combined lint+format) instead, which every `packages/*`/`apps/*`/
  `frontends/*` package does define.
